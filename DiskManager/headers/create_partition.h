...
//     }
//     char command[256];
//     sprintf(command, "sudo echo -e \"n\np\n%d\n\n+%dG\nw\nq\" | sudo fdisk %s > /dev/null", partition_number, partition_size, manager->disque_actif->device);
//     system(command);
...
void create_partition(struct DiskManager* manager, const char* file_system_type, int partition_number, int partition_size) {
    ...
    system(command);
    if (strcmp(file_system_type, "ntfs") == 0) {
        sprintf(command, "sudo mkfs.%s -f %s%d > /dev/null", file_system_type, manager->disque_actif->device, partition_number);
    } else if (strcmp(file_system_type, "ext4") == 0) {
        sprintf(command, "sudo mkfs.%s -F %s%d > /dev/null", file_system_type, manager->disque_actif->device, partition_number);
    } else {
        printf("Type de système de fichiers non pris en charge.\n");
